...
<div class="bg-white rounded-xl shadow-sm p-4 mb-6 animate-slideUp">
    <form method="GET" class="flex flex-col sm:flex-row flex-wrap gap-3">
        ...
        <a href="{{ route('admin.aspirasi.histori') }}" class="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all duration-300 text-center font-medium">Reset</a>
    </form>
</div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50/80">
                <tr>
                    ...
                    <th class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Selesai Pada</th>
            ...
            <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="row-hover">
                        ...
                        <td class="px-4 lg:px-6 py-4">
                            <div class="flex flex-col sm:flex-row gap-2">
                                ...
                                @if(!$item->is_archived)
                                    <form action="{{ route('admin.aspirasi.archive', $item->id) }}" method="POST" 
                                        onsubmit="return confirm('Apakah Anda yakin ingin mengarsipkan aspirasi ini?')">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-gray-500 hover:text-red-600 text-sm transition-colors duration-200 hover:underline">
                                            Arsip
                                        </button>
                                    </form>
                                @endif
                ...
                @empty
                    <tr><td colspan="6" class="px-6 py-8 text-center text-gray-500">Belum ada histori aspirasi selesai</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
